...
class GausUniffMixture:
    ...
    def density(self, x):
        ...
        for i in range (self.n_mixture):
            ...
            unif_density = 1 / (unif_b - unif_a) * (x <= unif_b) * (x >= unif_a)

            # Calculate ith gaussian density at x
            gaus_density = np.exp(- ((x - center) / sigma) ** 2 / 2) / (np.sqrt (2 * math.pi) * sigma)
...
class GU (Dataset):
    def __init__( self, dataset_size=100000, gu_num=8, device="cpu" ):
        ...
        self.device = torch.device (device)
        self.gu = GausUniffMixture (n_mixture=n_mix, mean_dist=mean_dist, sigma=gu_std, unif_intsect=uc, unif_ratio=ur,
                                    centralized=False, device=self.device, seed=1)
        self.input_size = 2
        self.label_size = 1
